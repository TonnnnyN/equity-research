...
Use `defaults/targets.toml` when the task is an Agent Skill research task and no live runtime config is needed. Use `config/watchlist.toml` when running the actual CLI pipeline.
