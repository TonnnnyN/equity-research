...
from market_sentiment.models import Benchmark, Layer, Security, Threshold
...
class RedditConfig:
    ...
    user_agent: str = "market-sentiment-social/0.1"
...
def load_config(config_path: str | None = None) -> ProjectConfig:
    config_override = config_path or os.environ.get("MARKET_SENTIMENT_CONFIG")
    ...
    data_dir = Path(os.environ.get("MARKET_SENTIMENT_DATA_DIR", default_data_dir))
    db_path = Path(os.environ.get("MARKET_SENTIMENT_DB_PATH", project["db_path"]))
    ...
    security_block = raw.get("securities", raw.get("tickers", []))
    securities = [
        Security(
            ticker=security["ticker"],
            name=security["name"],
            layer=Layer(security["layer"]),
    ...
    return ProjectConfig(
        ...
        default_user_agent=project.get(
            "default_user_agent",
            "market-sentiment-pipeline/0.1 your-name your-email@example.com",
        ),
        data_dir=data_dir,
        db_path=db_path,
...
def _load_email_config(email_block: dict) -> EmailDeliveryConfig:
    smtp_host = _first_non_empty(
        os.environ.get("MARKET_SENTIMENT_SMTP_HOST"),
        email_block.get("smtp_host"),
    )
    smtp_port = _parse_int(
        _first_non_empty(
            os.environ.get("MARKET_SENTIMENT_SMTP_PORT"),
            email_block.get("smtp_port"),
        ),
        default=587,
    )
    username = _first_non_empty(
        os.environ.get("MARKET_SENTIMENT_SMTP_USERNAME"),
        email_block.get("smtp_username"),
    )
    password = _first_non_empty(
        os.environ.get("MARKET_SENTIMENT_SMTP_PASSWORD"),
        email_block.get("smtp_password"),
    )
    from_address = _first_non_empty(
        os.environ.get("MARKET_SENTIMENT_EMAIL_FROM"),
        email_block.get("from_address"),
    )
    recipients = _parse_recipients(
        os.environ.get("MARKET_SENTIMENT_REPORT_EMAIL_TO")
    ...
    use_ssl = _parse_bool(
        _first_non_empty(
            os.environ.get("MARKET_SENTIMENT_SMTP_USE_SSL"),
    ...
    use_starttls = _parse_bool(
        _first_non_empty(
            os.environ.get("MARKET_SENTIMENT_SMTP_USE_STARTTLS"),
    ...
    timeout_seconds = float(
        _first_non_empty(
            os.environ.get("MARKET_SENTIMENT_SMTP_TIMEOUT_SECONDS"),
    ...
    subject_prefix = str(
        _first_non_empty(
            os.environ.get("MARKET_SENTIMENT_EMAIL_SUBJECT_PREFIX"),
