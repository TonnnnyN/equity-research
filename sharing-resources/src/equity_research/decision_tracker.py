...
from equity_research.decision_schema import VALID_COMPARATORS, validate_decision_payload
...
def evaluate_decision(
    ...
    *,
    ...
    """
    Evaluate a decision against fresh market data on a single day.

    This evaluates conditions as of run_date using all bars available up to that date.
    For day-by-day evaluation over a date range (to catch conditions firing during gaps),
    use evaluate_decision_through_date instead.
...
def evaluate_decision_through_date(
    decision: dict,
    *,
    bars: list[PriceBar],
    earnings: EarningsCalendar | None,
    since_date: date,
    until_date: date,
) -> tuple[DecisionAlert | None, date | None, float | None]:
    """
    Evaluate a decision day-by-day over a date range and return the first day a condition fires.

    This function iterates through each trading day from since_date (exclusive) through until_date (inclusive)
    and checks conditions on each day. It resolves on the first day any condition fires.

    Args:
        decision: The decision payload (dict with ticker, decision_date, conditions, etc.)
        bars: All available price bars, sorted ascending by trading_date.
        earnings: Optional earnings calendar data.
        since_date: Start of the date range (exclusive; evaluation begins the day after).
        until_date: End of the date range (inclusive).

    Returns:
        (alert, fired_date, fired_price) where:
        - alert: DecisionAlert if any condition fired, None otherwise.
        - fired_date: The date on which the alert fired (if alert is not None).
        - fired_price: The close price on that date (if alert is not None).
    """
    try:
        decision_date = date.fromisoformat(decision.get("decision_date", ""))
        reference_close = float(decision.get("reference_close", 0))
    ...
    # Sort bars by trading_date (defensive, should already be sorted).
    sorted_bars = sorted(bars, key=lambda b: b.trading_date)

    # Collect all trading dates in the range (since_date, until_date].
    trading_dates = sorted({b.trading_date for b in sorted_bars if since_date < b.trading_date <= until_date})

    if not trading_dates:
        return None, None, None

    # Track the earliest date any condition fires.
    earliest_fired_date: date | None = None
    earliest_fired_kind: str | None = None
    earliest_fired_condition: dict | None = None

    for current_date in trading_dates:
        # Build bars list up to and including current_date.
        bars_up_to_now = [b for b in sorted_bars if b.trading_date <= current_date]
        ...
        for condition in invalidate_conditions:
            if evaluate_condition(
                ...
                # Invalidate condition fired on current_date.
                if earliest_fired_date is None or current_date < earliest_fired_date:
                    earliest_fired_date = current_date
                    earliest_fired_kind = "invalidated"
                    earliest_fired_condition = condition
                # Don't break; we want to check all dates to find the earliest firing date.
                break

        # Only check rerate if we haven't already found an earlier firing date.
        if earliest_fired_date is None or current_date <= earliest_fired_date:
            for condition in rerate_conditions:
                if evaluate_condition(
                    condition,
                    ...
                    # Rerate condition fired on current_date.
                    if earliest_fired_date is None or current_date < earliest_fired_date:
                        earliest_fired_date = current_date
                        earliest_fired_kind = "rerated"
                        earliest_fired_condition = condition
                    # Don't break; we want to check all dates to find the earliest firing date.
                    break

        # If we found a firing date and processed past it, we can stop.
        if earliest_fired_date is not None and current_date > earliest_fired_date:
            break

    # If a condition fired, construct the alert.
    if earliest_fired_date is not None and earliest_fired_kind is not None:
        # Get the close price on the fired date.
        fired_bar = next((b for b in bars_up_to_now if b.trading_date == earliest_fired_date), None)
        fired_price = fired_bar.close if fired_bar else None

        # Compute the metric value as of the fired date for the reason string.
        metric_name = earliest_fired_condition.get("metric", "?") if earliest_fired_condition else "?"
        bars_up_to_fired = [b for b in sorted_bars if b.trading_date <= earliest_fired_date]
        metric_value = compute_metric(
            metric_name,
            bars=bars_up_to_fired,
            reference_close=reference_close,
            decision_date=decision_date,
            run_date=earliest_fired_date,
            earnings=earnings,
            as_of_index=-1,
        )
        if metric_value is None:
        ...
        reason = _format_reason(earliest_fired_condition, metric_value, metric_name) if earliest_fired_condition else "condition fired"

        alert = DecisionAlert(
            ticker=ticker,
            decision_date=decision.get("decision_date", ""),
            kind=earliest_fired_kind,
            reason=reason,
            fired_condition=earliest_fired_condition,
        )
        return alert, earliest_fired_date, fired_price
