...
class PipelineTests(TestCase):
    ...
    def test_track_decisions_fires_invalidate_condition(self) -> None:
        """Test that an active decision fires its invalidate condition and is marked invalidated."""
        with tempfile.TemporaryDirectory() as tmp:
            os.environ["MARKET_SENTIMENT_DATA_DIR"] = tmp
            os.environ["MARKET_SENTIMENT_DB_PATH"] = str(Path(tmp) / "market_sentiment.db")
            pipeline = DailyPipeline(load_config(str(CONFIG_PATH)))
            ...
            # Pre-insert an active decision (reference_close=100, invalidates when price > 110, i.e., 10% up)
            pipeline.storage.upsert_active_decision(
                ticker="MSFT",
                decision_date="2026-03-10",
                state="WATCH",
            ...
            def fake_prices(ticker: str, run_date: date, **_kwargs) -> SourcePayload[list[PriceBar]]:
                ...
                return SourcePayload(
                    data=bars,
                    status=SourceStatus(source="test", success=True, message="ok"),
                )

            pipeline.alpha_vantage.fetch_daily_prices = fake_prices  # type: ignore[method-assign]
            pipeline.tiger.fetch_daily_prices = fake_prices  # type: ignore[method-assign]
            ...
            self.assertEqual(result.alerts[0].ticker, "MSFT")
            self.assertEqual(result.alerts[0].kind, "invalidated")

            # Assert: decision status is now "invalidated"
            all_for_msft = pipeline.storage.read_all_decisions_for_ticker("MSFT")
            self.assertEqual(all_for_msft[0]["status"], "invalidated")
